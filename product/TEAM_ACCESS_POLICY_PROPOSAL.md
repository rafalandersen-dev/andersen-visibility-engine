...
## Preview assembly admission and complete image review cycles

A private `project_team_preview_limits` table admits assembly only after current account, project and membership checks. Actor budgets allow two active assemblies, 60 starts/minute and 240/hour; owner budgets allow eight active assemblies, 240/minute and 1,200/hour. Both scopes are acquired atomically without waiting for advisory/row locks. Failed owner admission rolls back actor consumption. Leases expire after 60 seconds, release only after underlying work ends, and assembled results older than 45 seconds are refused. Existing context/authority checks still run after admission. Only explicit capacity and lock contention use retryable publication preflight; permission uncertainty retains its hold.

Media's 512 starts/minute cover initial 128-image rendering, one refresh, approval verification and publication verification; four concurrent leases, the 600/hour ceiling and existing byte/pixel limits remain. Actual SQL tests exercise all four passes and the following capacity rejection. Preview changes passed the full 3,321-test / 247-file suite and production build; the final quota follow-up passed 99 focused tests, types and changed-file lint. These checks do not establish real multi-session acceptance or a production release.
