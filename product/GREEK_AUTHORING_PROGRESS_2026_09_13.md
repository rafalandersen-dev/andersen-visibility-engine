...
Greek is staged only: 1,488 of the current 3,768 English interface keys across eighteen complete batches. It is excluded from UI_CATALOGS and the language picker. Four UI catalogs remain active; thirteen other catalogs are completely authored in staging with acceptance still incomplete.
...
The frozen registry assigns each key once. Namespace checks verify exact keys, current composed-English source fingerprints, nonempty values and preserved placeholders, numbers, URLs and email tokens. All 29 Greek/catalog tests, scoped lint, formatting, whitespace and type checking pass. Type log: /tmp/milo-greek-configuration-types.log. Full-catalog equality will be required when all batches are authored.
