...
1. **Repair → first accepted turn (first).** The earlier PR136 gates are now cleared: PR136 is **merged** (`5226146434f26ea8410814efa2738bb520b68957`), its **8** Sep-13/14 chat/team migrations are **installed and verified**, and the build is deployed. What remains is the **repair**: the first real conversation **failed before any model reservation**, so **dispatcher and cron are disabled**, Claude's bounded repair is **ongoing** and signed-in real-use acceptance is **still pending**. Do **not** delay this for research, **never replay the installed migrations**, and do **not** re-enable dispatch before the repair is accepted.
...
**Positions.** Keep **B** (manager over one model, deterministic guards), collapsing to A for ordinary turns; **reject C for Milo** on current need. The four specialist families are **built and tested** (project-knowledge PR108, backlink-monitoring PR125 deployed) — not "missing"; the executor is now **merged and deployed** via PR136 (`5226146434f26ea8410814efa2738bb520b68957`) but its **conversation dispatch is not yet accepted** — the first turn failed before a model reservation and dispatcher/cron are disabled — alongside its owner-account payer. Routing is doubly inert (needs resolver + call-site + pricing code). The schema-enforced **≤5 native-text calls/turn** is ratified from the code trace — no new guard. The genuine gap is **measurement**; safety, provenance, memory, isolation, approval and idempotency rules need **runtime verification, not redesign**.

**Remaining dependencies.** PR136 is **merged** (`5226146434f26ea8410814efa2738bb520b68957`), its **8** chat/team migrations **installed and verified** (never replayed) and deployed — but the first real conversation **failed before a model reservation**, so **dispatcher and cron stay disabled**, Claude's repair is **ongoing** and **real acceptance is pending**; runtime acceptance of the self-evidence control (SQL ≠ acceptance); real per-accepted cost (unmeasured; unknown ≠ zero); a **fresh** native image gen (unproven Sep-19; the four Butelki items are recovered publications); the Sep-19 job `d6747d28…` cited from the review, not re-verified in-repo; Milo provider entitlement/pricing. No measured cost, provider result, image proof or production readiness is asserted; no numerical saving without measurement; new spend beyond standing authorization is an owner decision. Citation Intelligence (D03) is a separate parallel memo.
