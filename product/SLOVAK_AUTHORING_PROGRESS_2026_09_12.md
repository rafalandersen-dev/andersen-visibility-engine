...
Focused terminology/rendered checkpoint after 5cd8e2a: the pipeline In review status now uses Na kontrole, matching editor and marketplace status labels; Na kontrolu denotes a direction rather than the current state. Network listing/error wording uses zápis consistently. All eight knowledge-review browser interaction groups passed with actual Slovak staged text and local fixtures. Dates displayed in Slovak, panel language and evidence-description relationships passed harness assertions. Temporary server stopped. Full-page styling, responsive layout, fluent-language and actual screen-reader acceptance are not established. See scripts/knowledge-review-browser/README.md.
