...
Status: daily monitoring is technically released through PR125 at18:35UTC11September; configured-provider and signed-in acceptance remain open. See [release evidence](../evidence/backlink-monitoring-release-2026-09-11.md). Historical implementation entries below are superseded by that release record. Overall delivery remains approximately 55%, implementation approximately 70%. The existing backlink profile/gap implementation does not provide dated new/lost history. This work fills that registered gap without presenting aggregate index counts as verified individual placements.
...
## Bind collection to the displayed saved website

Explicit collection now includes an expectedWebsite precondition from the displayed project. The server compares its trimmed value with the authoritative saved website before any request reservation or expense admission. A mismatch returns website_changed without dispatch, with localized instructions to save or reload. The target continues to be derived only from the database; a client-supplied expected website cannot override it. Reservation and dispatch still recheck the original saved website. The form trims once before protocol detection and URL parsing, matching server handling.
