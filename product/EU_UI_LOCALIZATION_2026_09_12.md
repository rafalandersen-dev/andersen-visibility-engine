...
Released PR131 contains a browser-independent interface catalog, literal interpolation, 11 source-copy corrections in the four existing languages, a complete first French translation of the current 2,702 dictionary keys, and 26 displayed date/number calls bound to the selected interface language. French is staged outside the runtime and language picker. Dictionary coverage is not fluent-user approval, complete screen coverage or a delivered French interface. Overall delivery remains approximately 60%, implementation 75% (weighted 58.25% / 73.5% after PR130); no further milestone credit is assigned to this foundation release. See [verified release evidence](../evidence/ui-catalog-release-2026-09-12.md).
...
A conservative source inventory scanned 161 route/component files and found 1,083 candidate displayed strings in 66 files, plus 52 formatting calls, initially 29 of which used the device default rather than the chosen interface language. This is a triage inventory, not an exact remaining translation count: candidates can be shared words or deliberate proper names, and the scan does not exhaust conditional strings, shared data arrays, shared formatting helpers or server-returned messages. The largest candidate groups include the editor, plan, beta validation, public home page, AI visibility, pricing and audit screens. Initial evidence: `/tmp/milo-ui-embedded-before-formatting.json`; current inventory: `/tmp/milo-ui-embedded-inventory.json`.

The first formatting pass fixes 26 displayed calls across 13 screens/components: source and knowledge review history, generation recovery, technical inspection/ownership, outreach history and follow-ups, pending actions, analytics and backlink/placement views. They use the same selected interface language as their surrounding text. Pure child views receive the locale alongside their translation function. Timestamp values, local timezone behavior, UTC evidence windows, monetary amounts, currency choices, expiry checks and all dispatch/approval/accounting logic remain unchanged. Three default-format calls remain in the shared calendar/chart components; no current app imports of those components were found. Wider shared-helper, currency and date-fns formatting still needs review.

Formatting validation: 73 focused checks across 11 files and full TypeScript passed. The complete local run passed 4,335 tests but timed out in two existing five-second weekly-executor/team-history cases (303 files, 4,337 tests total); it is not recorded as a clean full-suite run. Both affected files plus all affected localization/render checks then passed with one worker and unchanged deadlines: 180 checks across 13 files. The production build passed. Final lint finds no diagnostics on edited lines; 45 pre-existing style diagnostics remain in the actions screen and 41 in the marketplace screen (46 there before this change). Artifacts: `/tmp/milo-ui-formatting-{tests,types}.log`, `/tmp/milo-ui-catalog-full-tests.log`, `/tmp/milo-ui-formatting-final-tests.log`, `/tmp/milo-ui-catalog-build.log`, and `/tmp/milo-ui-formatting-lint-final-summary.json`. Dedicated CI34671978059 subsequently passed all4,337 tests/303 files, types/builds/frozen locks on both runtimes. Actual browser acceptance remains outstanding.

Next: extract the editor's remaining displayed copy with precise context, review shared formatting helpers, complete the other 19 new interface languages, and implement selected-language loading without shipping all 24 dictionaries eagerly. Keep content, email, interface, market, currency and ownership preferences separate. Retain explicit UTC evidence windows and actual currency units when formatting. French remains unavailable until required screen coverage, language selection, runtime behavior and fluent-user acceptance are completed. Public policy review, signed-in journeys, actual reports and full R20 acceptance remain open.
