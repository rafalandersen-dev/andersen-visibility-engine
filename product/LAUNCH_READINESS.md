...
> **Superseded 2026-09-19 (application status only):** `20260912040000_knowledge_review_batch_time.sql` was **applied to production** in the PR136 release (merge `52261464`) as one of the 8 migrations in that release packet, with the exact migration hashes verified in that packet. The failed chat/conversation dispatcher remains **OFF**; that release enabled no dispatcher. Do **not** reapply this SQL — it is already applied. Details and the verified hashes are recorded in [`evidence/conversation-production-release-2026-09-19.md`](../evidence/conversation-production-release-2026-09-19.md) (present on current `main`; this branch's older base predates that file and will pick it up when this stage merges after `main`). This updates only the migration's application status; it grants no new release permission and makes no public-launch claim. The 12 September pre-release assessment below is retained for the record.

`20260912040000_knowledge_review_batch_time.sql` was, per the 12 September pre-release assessment retained here, committed candidate work not yet applied or approved for production. It replaces only `read_output_knowledge_review_batch`; it does not change tables, response fields or stored review history. It excludes future-dated and withdrawn entries from the active-review result while preserving history visibility and service-role-only execution.
