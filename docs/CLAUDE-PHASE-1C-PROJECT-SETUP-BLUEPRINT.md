...
- **Preview:** the server-derived markdown, plus a structured render:
  - *Business profile* — field-level **current → proposed** diff (project fields), computed live at render against the current project. Every provided field carries an explicit textual state marker: **New** (empty current filled), **Overwrites current** (a non-empty value replaced — decision §12.3), or **No change** (proposed equals current after normalization). Array/set fields (`targetLocations`, `additionalLanguages`, `competitorUrls`) normalize by trim + case + dedupe + sort, so equivalent sets read as *No change* rather than a false overwrite.
    - **Changed-target note (corrected 1C.4):** there is **no** "target has changed since this was suggested" warning, and none was ever implemented in 1B — proposals store no baseline field values or workspace `rev`, so a change *since creation* is not detectable, and this patch does not invent a revision architecture to fake it. The truthful substitute is the **live** current→proposed diff above: the owner always sees the real current value at approval time. The only target warning is the deleted-project case (below).
  - *Services to create* and *Opportunities to create* — additive lists (no diff), with per-item title/kind/priority, plus a disclaimer that these are proposed new items and that duplicates / over-cap items are skipped on approval (counts are not guarantees).
  - *Competitors* — the proposed `competitorUrls` list with an **Adds / Overwrites current / No change** marker vs the project's current competitors (the apply replaces the whole list). **Rendered as plain text, never as clickable links** (proposal content is untrusted input).
  - *Deleted target* — if the project no longer exists, a `role="status"` warning renders and **Approve & apply is disabled** (Reject stays available) so the UI never encourages an apply the server would fail-close on.
