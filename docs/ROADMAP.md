...
| **P0.4** | Internal‑link safety: three explicit states, publishing blocked while unresolved | S/M | Every relative in‑body internal link has exactly one state: **VERIFIED** (site root or a page Milo has published), **USER_APPROVED** (an exact path the user approved for the project, persisted in the project JSONB — no migration), or **UNRESOLVED**. VERIFIED and USER_APPROVED publish as active links; an UNRESOLVED link is **never sent as active and never silently removed** — publishing is **blocked** until the author resolves it (approve this exact URL / replace with a verified page / keep as plain text / remove). No global "approve all". The same contract governs **all three connectors** including the custom endpoint (the old `keepAllInternalLinks` escape hatch is removed), and preview/export/publish show the same effective state. Enforced at every layer: the editor blocks the send/publish/schedule buttons and lists each unresolved link (anchor, path, source section, reason); WordPress + Shopify refuse before any network call; the scheduled runner parks the row (`PublishNotPossibleError`); the browser store path refuses too. *Demonstrated* by `connector-link-safety.test.ts` + `store.link-safety.test.ts` (verified/approved through WP/Shopify/custom; invented and valid‑but‑unknown blocked through all three; republish keeps the self‑link but blocks a new invented one; parity; per‑path approval persists; no blanket approval). A sitemap‑based inventory of *existing* customer URLs remains P1.1. |
...
4. **P0.4 internal‑link safety (three‑state contract)** — `markdown.ts` classifies each relative in‑body link as VERIFIED / USER_APPROVED / UNRESOLVED (`classifyInternalLinks`) and exposes resolver rewrites (`replaceLinkPath`, `linkPathToText`, `removeLinkByPath`); the active set (verified ∪ approved) is built once in `publish-targets.ts` (`buildActiveInternalPaths`, `unresolvedLinksForPublish`) and passed to the canonical converter by preview, export **and** every connector. Publishing is refused while any link is UNRESOLVED — WordPress/Shopify guard before network, `publish.server.ts` parks the row, `mock-ai.ts` refuses the manual path, and the editor blocks send/publish/schedule while listing each unresolved link with approve/replace/keep‑as‑text/remove. Approvals persist per‑path on `project.approvedInternalPaths` (JSONB, no migration) via `approveProjectInternalPath`; there is no blanket approval. Tests: `connector-link-safety.test.ts`, `store.link-safety.test.ts`.
