-- Expand supported incident types; preserve recipient isolation and sync semantics.
...
ALTER TABLE public.operational_notifications ADD CONSTRAINT operational_notifications_kind_check
  CHECK (kind IN ('approval_due','publication_failed','manual_overdue','cadence_gap','scheduler_recovery'));
...
BEGIN
  ...
  FOR e IN SELECT value FROM jsonb_array_elements(p_events) LOOP
    IF jsonb_typeof(e)<>'object' OR e->>'projectId' IS NULL OR e->>'targetId' IS NULL
       ...
       OR e->>'kind' NOT IN ('approval_due','publication_failed','manual_overdue','cadence_gap','scheduler_recovery')
