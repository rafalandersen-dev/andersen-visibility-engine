...
BEGIN
 ...
 FOREACH kind IN ARRAY ARRAY['account','target','address'] LOOP
   ...
   IF (SELECT count(*) FROM jsonb_object_keys(active_leases))>=2 THEN RAISE EXCEPTION 'technical_dispatch_capacity'; END IF;
   ...
   IF budget.minute_count>=(CASE WHEN kind='account' THEN 120 ELSE 60 END) OR budget.hour_count>=(CASE WHEN kind='account' THEN 1200 ELSE 600 END) THEN RAISE EXCEPTION 'technical_dispatch_capacity'; END IF;
