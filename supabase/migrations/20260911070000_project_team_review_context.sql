...
BEGIN
  ...
  IF p_actor<>p_owner AND policy.mode='separate_reviewers' AND (SELECT actor_id FROM public.project_team_edits WHERE owner_id=p_owner AND project_id=p_project AND asset_id=p_asset AND after_hash=snapshot->>'draftHash' AND before_hash<>after_hash ORDER BY created_at DESC,edit_id DESC LIMIT 1)=p_actor THEN allowed:=false; END IF;
