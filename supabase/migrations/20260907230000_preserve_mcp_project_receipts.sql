...
BEGIN
  IF NEW.collection <> 'projects' OR auth.role() = 'service_role' THEN
    RETURN NEW;
  END IF;
  FOREACH receipt_field IN ARRAY ARRAY['mcpProfileFillRequests', 'mcpOpportunityBatches'] LOOP
...
CREATE TRIGGER preserve_mcp_project_receipts
BEFORE INSERT OR UPDATE ON public.workspace_entities
FOR EACH ROW EXECUTE FUNCTION public.preserve_mcp_project_receipts();
