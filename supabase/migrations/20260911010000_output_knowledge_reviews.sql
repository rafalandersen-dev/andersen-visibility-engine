...
-- One bounded project snapshot for the affected-draft list; no per-draft
-- network reads and no history disappearance after a current hold is cleared.
CREATE FUNCTION public.read_output_knowledge_review_batch(p_user uuid,p_project text,p_assets text[])
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
