...
Follow-up source-context checks retain the Search Console property with each observation. Only matching declared API properties can produce tentative deltas; manual CSV observations remain individually readable because their search filters are unavailable. Other publication attempts during either comparison window also make the result inconclusive. Focused observation/migration/locale tests and types pass.
