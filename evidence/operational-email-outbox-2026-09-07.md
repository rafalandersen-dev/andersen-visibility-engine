...
- A fresh, revision-matched server scan may atomically enqueue one aggregate per account per hour, at most 50 new incidents. Unique incident membership prevents repeat scans/retries from producing duplicate emails. Existing imported/read/resolved incidents are not converted to send events. Remaining eligible incidents can enter later bounded batches.
