...
## TS/SQL schedule-instant precision consistency — status UNRUN (P1 4058548632)

Round 25 (on f555e00) fixes a TS/SQL divergence in the round-24 schedule work. `stockholmParts`/`stockholmWallClock` dropped fractional seconds (whole-second Intl parts) while the SQL lock compares full-precision `timestamptz` local time (`loc::time`), so a schedule whose rounds differ in fractional seconds (round 1 at `.000`, round 2 at `.500`) passed `lockedPanelSchema` yet the SQL lock rejected it (surfacing as the generic unavailable error). Separately `Date.parse` is millisecond-precision while `timestamptz` is microsecond, so slot equality and delay could diverge on sub-millisecond variants.

- Explicit supported precision = MILLISECONDS, enforced identically in TS and SQL across ALL schedule paths — cadence validation, slot equality, delay, and the final-week window. TS: `stockholmFmt` gains `fractionalSecondDigits: 3`; `stockholmWallClock` compares MILLISECOND-of-day (so a differing fractional second is a real cadence difference, matching SQL); `stockholmOffsetMs`/`stockholmWeekLater` carry the millisecond so the last round's DST window is neither shifted nor stripped of its fraction. SQL: the lock cadence adds `ts = date_trunc('milliseconds', ts)` per slot, and `save_citation_capture` requires `intendedSlotAt`/`capturedAt` to equal their `date_trunc('milliseconds', …)`.
- Sub-millisecond precision is refused at NEW admission (narrow): SQL lock (`ts <> date_trunc('milliseconds', ts)` → `citation_panel_schedule_invalid`), SQL capture (`intendedSlotAt` → `citation_intended_slot_mismatch`; `capturedAt` → `citation_delay_untruthful`), and `lockedPanelSchema` via `discoveryScheduleValid` (`isMillisecondPrecise`). A historical finer instant reads FAIL-CLOSED (`discoveryScheduleValid`/`discoveryScheduleDeviations` return invalid/off-schedule) — the stored document is never rewritten. This deliberately does not "solve only .001": a `.000001` variant is refused rather than silently truncated, so TS and SQL cannot disagree on it. A consistent fractional second (e.g. all-`.500`) remains a valid weekly cadence and round-trips verbatim.
- Preserved: immutable historical documents (no rewrite), truthful comparisons, brand-unscheduled separation, failed/missed/truncated evidence, and every round-24 invariant (one baseline / UUID / idempotency / erasure / caps). Edited only `supabase/migrations/20260920190000_citation_protocol.sql` (candidate), `src/lib/citation-panel.ts` (precision helpers), and the three P2 test files (`citation-panel.test.ts`, `citation-protocol.test.ts`, `citation-protocol-migration.test.ts`) plus these docs. No SQL applied, no P3, no provider, no commit/deploy.

The last PASSING baseline (Codex on f555e00: 193 focused / 6357 full across 385 files PASS, 105.13s, types/scoped-lint/build PASS) does NOT carry over; every round-25 check is UNRUN in this worktree and must be re-executed by Codex under the recorded exception — this assistant did not run tests and claims no PASS. New checks: pure helpers — same-fraction valid, differing-fraction invalid, sub-millisecond invalid; equivalent-offset with a fraction on-schedule; the DST last-round window with a `.500` fraction (within vs at the wall-clock boundary). `lockedPanelSchema` — same-fraction locks, differing/sub-millisecond rejected. Resolver — a sub-millisecond capture demotes (fail closed). Real SQL — the lock rejects differing-fraction and sub-millisecond schedules and locks a same-fraction one (fraction preserved on read); the capture RPC refuses a sub-millisecond `intendedSlotAt` (mismatch) and `capturedAt` (delay-untruthful), proving TS and SQL agree. Candidate `20260920190000` remains UNAPPLIED; prepared deploy SQL / expected-identity artifacts remain STALE; USD50 + manual-free budget unchanged; a real pilot still requires genuine owner approval and a real four weeks; nothing is deployed. Test commands: `npx vitest run src/lib/citation-panel.test.ts src/lib/citation-protocol.test.ts src/lib/citation-protocol.functions.test.ts src/lib/citation-protocol-migration.test.ts`; `npx tsc --noEmit`; and the SCOPED lint on the touched files (the repo-wide lint is separately red and pre-existing).

## Codex schedule-precision verification — 21 September 2026

Reviewed millisecond cadence, slot and delay comparisons plus fractional DST boundary preservation in TS/SQL. Focused199tests/4filesPASS5.65s, TypeScriptPASS. Scoped ESLint, full6363tests/385filesPASS40.89s, production build and git diff --checkPASS. Logs /tmp/milo-p2-precision-{focused,types,format,lint,full,build}-20260921.log. Formatting-only Codex integration exception on four changed TS files. Historical finer precision is flagged, not rewritten. Exact-head reviews and production acceptance remain outstanding; candidate20260920190000UNAPPLIED and prior guarded SQL/identity artifactsSTALE.

## Malformed auxiliary captureContext surfaced as explicit invalid evidence — status UNRUN (P2 4058641656)

Round 26 (on `033e94e6`) fixes a silent-drop in `resolveStoredCaptures`. `captureContext` is stored OPAQUELY (`answerEvidenceSchema` keeps it a bounded JSON value), so a PRESENT-but-malformed context passes the SQL/server state read (`citationProtocolStateSchema.answers`) and reaches the resolver, where the STRICT `captureContextSchema` parse fails. The pre-fix branch did `if (!parsed.success) continue;` — silently dropping the row even when its panel/run/slot identity was recognizable. A discovery slot then read a false `neverObserved`, and a brand attempt the authoritative SQL `runConsumed` aggregate already counted was hidden (consumed with no visible observation). Legacy context-LESS rows (null/undefined) are a separate case: they are ordinary answer evidence, never captures, and are still skipped at the null guard.

- The malformed branch now surfaces the row as explicit INVALID evidence carrying ONLY recognizable scoped identity — nothing is invented to fit the schema. It extracts `panelId`/`panelVersion`/`brandRunId` from the raw context (typeof + `PG_UUID_RE` guards), resolves the panel by canonical uuid against the locked+approved panels, and recognizes the slot ONLY when `questionId` is a real grid question of that panel AND `round` is in range for the kind/run. It pushes `outcome: protocol_deviant` with deviations `malformed_capture_context` (+ `panel_unresolved` when the panel is unrecognizable, + `unknown_slot` when the panel is known but the slot is not), `contextMalformed: true`, the recognized `slot`/`brandRunId` (or null), and `captureContext: null`.
- Recognizable scoped identity → a KNOWN slot is `observed`-invalid: `citationReport` counts it as observed at that slot (never promoted to `complete`), so it occupies the slot and is not a false `neverObserved`; a brand row at a recognized run/slot increments `runObserved`, so `observed` coheres with the authoritative `consumed`. An UNKNOWN slot (panel recognized, slot not) is counted as `excluded` and sets a per-version `unknownAttempt` flag → `coverageComplete` false → `neverObserved` null (absence UNKNOWN, never a false definitive known-never-observed); the SQL `consumed` stays authoritative, and the consumed>observed gap IS the surfaced excluded attempt. An unrecognizable PANEL is an orphan (`panelResolved: false`) attributed to no per-panel report.
- `ResolvedCapture` gained `contextMalformed`, `slot: {questionId,round}|null`, `brandRunId: string|null`, and `captureContext` is now `CaptureContext | null`. The dedup/`slotKey` and `resolveErasedSlots` paths read the recognized `slot`/`brandRunId`. Live/erased slot keys stay the shared 5-tuple of canonical-uuid components, so live and erased facts for one slot still collapse together. The raw answer document/hash is never rewritten (still inspectable via `readAnswerEvidence`); caps/correction-leaf/tenant/scope/UUID-normalization/erasure/duplicate accounting and strict NEW writes (`save_citation_capture` still validates, `importAnswerEvidence` still refuses any context) are unchanged. Edited only `src/lib/citation-protocol.ts` and the two P2 test files (`citation-protocol.test.ts`, `citation-protocol-migration.test.ts`) plus these docs; `citation-panel.ts` and the candidate SQL are untouched (the read keeps captureContext opaque, so no SQL change is required). No SQL applied, no P3, no provider, no commit/deploy.

**Amendment (this turn) — determinism at a shared slot + folding a malformed correction.** A follow-up review confirmed two gaps in the first round-26 pass: (1) a malformed recognized-slot row skipped the duplicate-slot collapse, so a valid original and an INDEPENDENT malformed original at the SAME planned slot were order-dependent (`citationReport`'s first-wins slot dedup made the slot read `complete` or invalid by input order); and (2) the first pass surfaced a malformed CORRECTION as a phantom independent attempt. Both are fixed in `resolveStoredCaptures`, deterministically and without dropping raw history:
  - **Fold a failed correction.** A context-less/malformed successor whose `supersedesId` points at a still-surviving capture (present-context, not valid-superseded — tracked in a new `survivingCaptureIds` set) is a FAILED CORRECTION that re-describes that survivor's already-resolved observation. It is folded (skipped) — it never supersedes the survivor (only a valid successor does) and is never re-surfaced as an independent attempt, so the survivor stays its own outcome (e.g. `complete`), never demoted to a false `duplicate_slot` by a re-description of itself. A malformed row whose predecessor does NOT survive (independent original, or correction of a deleted/context-less/superseded row) is still surfaced. This also RECONCILES the round-3 regression (`citation-protocol.test.ts`): a malformed successor of a surviving capture folds, so the valid predecessor alone resolves — the `["cap"]` expectation holds WITHOUT dropping genuinely independent malformed rows.
  - **Collapse malformed independent originals too.** The slot-duplicate collapse now counts every panel-resolved capture with a RECOGNIZED slot (valid OR malformed) as a slot occupant, so a valid + INDEPENDENT-malformed pair at one slot deterministically collapses to ONE `protocol_deviant`/`duplicate_slot` entry in stable input order — the report reads observed 1 / `protocol_deviant` 1 / `complete` 0 / neverObserved 39 in BOTH orders. Orphans (no resolvable panel) and unknown-slot malformed rows carry no comparable slot and pass through unchanged.
- Determinism holds end-to-end because `resolveStoredCaptures` collapses same-slot occupants BEFORE `citationReport`, so the report never receives an ambiguous same-slot pair from the real pipeline; the report's own defensive first-wins dedup only ever sees the single collapsed entry.

The last PASSING baseline (Codex on `033e94e6`: 199 focused / 6363 full across 385 files PASS, types/scoped-lint/build PASS) does NOT carry over; the follow-up Codex run of the first round-26 pass was 207 focused PASS / 1 FAIL (the round-3 `["cap"]` regression, now reconciled by the fold above) with `tsc` NOT run (chained after the failure). Every round-26 check is UNRUN in this worktree and must be re-executed by Codex — this assistant did not run tests and claims no PASS. Checks: pure resolver/report unit — recognized-slot observed-invalid (observed 1, neverObserved 39, never `complete`); unrecognizable-slot excluded (`coverageComplete` false / `neverObserved` null, no invented absence); unrecognizable-panel orphan (touches no coverage); legacy context-less skipped vs malformed-present surfaced; malformed brand recognized-slot observed==consumed, brand unknown-slot excluded while consumed authoritative; a malformed correction of a surviving capture FOLDS (original stays `complete`, never demoted); a valid + INDEPENDENT-malformed pair at one slot collapses deterministically in BOTH input orders; a malformed independent original at a KNOWN erased slot flags `erased_duplicate_slot`. Real SQL (PGlite) — a stored row whose `captureContext` is corrupted via direct `jsonb_set` flows through `readResolvedCaptures` → `citationReport`: discovery recognized-slot (observed 1 / neverObserved 39) vs unknown-slot (excluded 1 / neverObserved null); brand recognized run/slot (consumed 1 / observed 1) vs unknown slot (excluded 1 / consumed 1 / observed 0) — with the raw document still inspectable. Candidate `20260920190000` remains UNAPPLIED; prepared deploy SQL / expected-identity artifacts remain STALE; USD50 + manual-free budget unchanged; a real pilot still requires genuine owner approval and a real four weeks; nothing is deployed. Test commands: `npx vitest run src/lib/citation-protocol.test.ts src/lib/citation-protocol.functions.test.ts src/lib/citation-protocol-migration.test.ts`; `npx tsc --noEmit`; and the SCOPED lint on the touched files (the repo-wide lint is separately red and pre-existing).

## Codex malformed-observation verification — 21 September 2026

Reviewed recognizable identity without invented capture context, unknown-slot coverage, deterministic valid/malformed duplicate accounting, failed-correction preservation and actual storage brand-consumption reporting. Four focused suites212PASS6.82s; TypeScriptPASS. Scoped ESLint, full6376tests/385filesPASS44.46s, production build and git diff --checkPASS. Logs /tmp/milo-p2-malformed-final-{focused,types,format,lint,full,build}-20260921.log. Formatter-only Codex integration exception on three changed TS files. Earlier207/1failed stale expectation is superseded; raw stored documents remain unchanged. Exact-head reviews and release acceptance outstanding; candidate20260920190000UNAPPLIED and prior guarded SQL/identity filesSTALE.

## Same-instant correction identity + exact live-extra attempt count — status UNRUN (P2 4058831146 / 4058831148)

Two independently-confirmed findings on `9484265f` (security 948 clean), one coherent packet.

- **4058831146 — correction capture-instant compared by VALUE, not raw text.** `save_citation_capture`'s correction-identity guard compared `pred_ctx->'time'->>'capturedAt' IS DISTINCT FROM ctx->'time'->>'capturedAt'` as RAW STRINGS, so a correction that re-spells the SAME capture instant with an equivalent offset (e.g. `2026-07-06T10:00:00.000+02:00` vs its `08:00:00.000Z` equivalent) was wrongly rejected as a different observation. Fix (candidate SQL only): a new content-free helper `public.citation_ts_ms(text)` parses a stored ISO instant to a `timestamptz` at the supported (round-25 millisecond) precision, or NULL. It FAILS CLOSED (returns NULL) on an unsupported sub-millisecond precision, a non-finite (±infinity) instant, or a malformed/absent value — it does NOT truncate a finer instant onto the millisecond (which would silently treat two genuinely different sub-millisecond instants as equal and admit a correction at a changed instant, per the independent delta on this round). It catches only the datetime SQLSTATEs (`invalid_datetime_format`/`datetime_field_overflow`/`invalid_text_representation`), mirroring `citation_ctx_run`, so a genuine fault still propagates. The guard now compares `citation_ts_ms(pred…) IS DISTINCT FROM citation_ts_ms(ctx…)`: an equivalent-offset re-spelling at millisecond precision is accepted, a genuinely different instant stays DISTINCT and is refused, and a malformed/sub-millisecond/non-finite HISTORICAL predecessor value parses to NULL and fails CLOSED (DISTINCT from the new capture's real instant) instead of raising inside the function (the pre-fix raw-string compare never cast, so the crash risk is introduced only by moving to a value compare — the helper contains it). This matches the round-25 admission guard (sub-millisecond refused at write), so a historical finer value reads fail-closed and no stored document is ever rewritten. The TS resolver already binds correction identity by chain id (`supersedesId` + valid parse), never by a raw `capturedAt` string, so it already accepts an equivalent-offset correction — no TS change; the fix is SQL-only and the two layers agree at millisecond precision.
- **4058831148 — expose the exact live-extra attempt count.** The resolver's duplicate-slot collapse dropped every additional live original silently (`if (seen.has(key)) continue;`), so three historical originals at one slot reported one deviant with NO extra count — the live analogue of the bug `erasedExtra` already fixed for tombstones. Fix (`citation-protocol.ts` only): the collapse survivor now carries `liveDuplicateExtras` = occupants − 1 (valid OR malformed independents at the recognized slot), and `citationReport` sums it per version into a new `liveExtra` field. Live captures are the WHOLE snapshot (bounded by the 100-answer cap, never LIMIT-truncated like tombstones), so the count is authoritative/exact. Three originals → `observed` 1 AND `liveExtra` 2; two → `liveExtra` 1. `liveExtra` is never a new planned slot, never double-counts the one occupant in `observed`, and (like `erasedExtra`) is never folded into `excluded`; a lone capture or a correction chain (one active leaf) contributes 0. The resolved list still collapses to one entry per slot (so `panelCounts` never trips), determinism (valid+malformed collision) and the failed-correction fold are preserved, and live+erased extras are counted independently with no double-count and no false coverage.
- Scope: only `supabase/migrations/20260920190000_citation_protocol.sql` (candidate — the `citation_ts_ms` helper + the value compare) and `src/lib/citation-protocol.ts` (`ResolvedCapture.liveDuplicateExtras`, `CitationReport.liveExtra`) plus the two P2 test files and these docs. No applied SQL, no P3, no provider, no commit/deploy; caps/tenant/scope/UUID/erasure accounting, strict NEW writes and immutable docs/hashes unchanged.

The prior PASSING baseline (Codex on `9484265f`: 212 focused / 6376 full across 385 files PASS, types/scoped-lint/build PASS) does NOT carry over; the follow-up Codex run of the first pass was **217 focused PASS / 4 FAIL** with `tsc` NOT run (chained after the failure): three correction tests asserted the internal `citation_correction_identity_mismatch` against the wrapper (which intentionally maps every RPC error to the generic `citation_protocol_unavailable`), and the empty-report `toEqual` predated the new `liveExtra`; plus the independent delta that `citation_ts_ms` truncated sub-millisecond history. All reconciled this turn: the specific error is now asserted against the RAW RPC (`db.query(SELECT save_citation_capture …)`) while the public contract is asserted to map to the generic error (the wrapper is NOT weakened); `liveExtra: 0` added to the empty-report shape; and the helper now fails closed on sub-millisecond/non-finite rather than truncating. Every check here is UNRUN in this worktree and must be re-executed by Codex — this assistant did not run tests and claims no PASS. Checks: real SQL (PGlite) — an equivalent-offset correction at `.000` AND at a `.500` fractional second is ACCEPTED (chain resolves to the leaf); a genuinely different instant, a corrupted-date predecessor, and a SUB-MILLISECOND predecessor are each REFUSED with the SPECIFIC `citation_correction_identity_mismatch` on the raw RPC while the wrapper maps to `citation_protocol_unavailable`; the sub-millisecond case also asserts the stored predecessor document is never rewritten. Pure resolver/report — N live originals at one slot → observed 1 / `liveExtra` N−1 / never `complete`; a correction chain → `liveExtra` 0; a valid + INDEPENDENT-malformed collision → observed 1 / `liveExtra` 1 deterministically in both orders. Real SQL canonical read→report — three historical live originals → observed 1 / `liveExtra` 2 with all three raw rows kept; a valid + malformed independent original at one slot → observed 1 / `liveExtra` 1; a live correction chain → `liveExtra` 0; live extras at one slot AND erased extras at another (SY-D01 r1 vs r2) → `liveExtra` 1 + `erasedExtra` 1, `recorded` 2, `neverObserved` 38, no double-count. Candidate `20260920190000` remains UNAPPLIED; prepared deploy SQL / expected-identity artifacts remain STALE; USD50 + manual-free budget unchanged; a real pilot still requires genuine owner approval and a real four weeks; nothing is deployed. Test commands: `npx vitest run src/lib/citation-protocol.test.ts src/lib/citation-protocol.functions.test.ts src/lib/citation-protocol-migration.test.ts`; `npx tsc --noEmit`; and the SCOPED lint on the touched files (the repo-wide lint is separately red and pre-existing).

### Independent verification — 2026-09-21, correction instants and live extras

223 focused tests across four files and 6387 full tests across 385 files passed (full run 48.10s). TypeScript, scoped ESLint, production build and git diff --check passed. Reviewed same-instant SQL comparisons with fail-closed unsupported precision and exact live duplicate extras. Codex exception: formatter-only changes on the three changed TypeScript files and this verification checkpoint. Candidate remains unapplied; no production, concurrent-connection, consumer measurement or provider-cost proof is implied.

## Intake capture-instant by value + bounded account-tier metadata — status UNRUN (P2 4058994517 + spec §5.2)

On `14672bdb`. One coherent packet closing a confirmed finding and an accepted spec gap from review 5262345901.

- **4058994517 — initial-intake capture-instant compared by VALUE.** The self-consistency check that a record's top-level `input.capturedAt` equals its `captureContext.time.capturedAt` compared RAW STRINGS in BOTH `parseManualCaptureInput` (TS) and the SQL `save_citation_capture` guard, so an equivalent-offset re-spelling of one instant (`2026-07-06T10:00:00.000+02:00` vs its `08:00:00.000Z` equivalent) was wrongly rejected, while an unsupported sub-millisecond value that matched byte-for-byte was wrongly admitted. Fix: both layers now compare by value at the supported (millisecond) precision. TS uses a new exported `supportedInstantMs(iso)` — the twin of the SQL `citation_ts_ms(text)` — returning the millisecond epoch iff the string is finite AND millisecond-precise, else null (fail closed on sub-millisecond/non-finite/malformed). SQL reuses `citation_ts_ms` with an explicit `IS NULL` check (so two equally-unsupported strings — both → NULL, `NULL IS DISTINCT FROM NULL` false — also fail closed). An equivalent offset (incl. a `.500` fraction) is accepted; a genuinely different instant, or an unsupported/non-finite/malformed value on either field, is refused. The raw stored strings are never rewritten — only the comparison is by value. The already-fixed correction guard uses the same helper, so intake and correction identity are consistent across TS and SQL. No new SQL object (the intake guard reuses `citation_ts_ms`); the rollback inventory is unchanged.
- **Spec §5.2 account-tier gap (from review 5262345901).** §5.2 requires recording the consumer "account tier if known" (explicitly: no passwords, cookies, account email or session tokens), but the strict session schema had no field. A follow-up product note observed that a coarse `free/paid` enum loses the ACTUAL tier (Plus vs Pro are both paid) and mis-reports comparability, so the property `accountTier` is a single OPTIONAL bounded LABEL — typed by the `accountTierLabel` schema: trimmed, non-blank, ≤40 chars, no control characters, no fixed provider list — on BOTH the panel-intended `sessionProtocolSchema` and the capture-context session. The bound constrains SYNTAX only and rejects blank/oversized/control values; it does NOT detect secrets — by intended-use instruction (spec §5.2) it records only the tier, not passwords/account ids/emails/session tokens, and no secret-detection mechanism is added. Optional = backward-compatible: a historical panel/capture with no field still parses (treated as not-known); the literal "unknown" is the explicit not-known value; no prior tier is invented. `protocolDeviations` flags `account_tier_differs` ONLY when BOTH the panel's intended tier and the capture's tier are known (present and not "unknown") and the EXACT labels differ (Plus vs Pro deviates) — an absent/"unknown" tier on either side is never a mismatch, so historical/not-known captures stay comparable and a known-tier capture's exact label is retained verbatim. One field is adequate (no redundant coarse-class field). The SQL stores the capture context opaquely (bounded JSON), so no SQL change is needed; product is not expanded and no guard is weakened.
- Scope: `src/lib/citation-panel.ts` (`supportedInstantMs`; `accountTier` on both sessions; the tier comparison), `src/lib/citation-protocol.ts` (intake value compare via `supportedInstantMs`), `supabase/migrations/20260920190000_citation_protocol.sql` (intake guard now `citation_ts_ms`-based, no new object), the three P2 test files and these docs. No applied SQL, no P3, no provider, no commit/deploy; caps/tenant/scope/UUID/erasure accounting, strict NEW writes and immutable docs/hashes unchanged; USD50 + manual-free budget unchanged.

The prior PASSING baseline (Codex on `14672bdb`: 223 focused / 6387 full across 385 files PASS, types/scoped-lint/build PASS, recorded above) does NOT carry over; the follow-up Codex run of the first round-28 pass was **229 focused / 228 PASS / 1 FAIL** with `tsc` NOT run: the round-25 sub-millisecond CAPTURE test still expected `citation_delay_untruthful`, but the round-28 intake guard raises the earlier `citation_capture_time_mismatch` — that expectation is updated to the truthful guard (no-write assertion kept); and an independent product note (free/paid loses Plus vs Pro) upgraded the tier to a bounded actual LABEL — both reconciled this turn. Every check here is UNRUN in this worktree and must be re-executed by Codex — this assistant did not run tests and claims no PASS. Checks: TS admission (`parseManualCaptureInput`) — top-level and context capturedAt as equivalent-offset spellings of one instant at `.000` and `.500` ACCEPTED (raw bytes preserved), a genuinely different instant REFUSED, unsupported sub-millisecond REFUSED even when the two fields are byte-identical. SQL admission — an equivalent-offset capture is admitted through BOTH layers (bytes preserved); a different instant and a sub-millisecond value are each REFUSED with `citation_capture_time_mismatch` on the RAW RPC (`db.query(SELECT save_citation_capture …)`; the wrapper maps to the generic error, not weakened); the round-25 sub-millisecond capture test now truthfully expects `citation_capture_time_mismatch`. Tier — the bounded LABEL accepts actual tiers (Free/Plus/Pro/Enterprise) + the "unknown" sentinel + an absent field on both the panel and capture sessions, and refuses blank/whitespace-only, control-character and oversized (>40) values; `protocolDeviations` flags `account_tier_differs` only when both exact labels are known and differ (Plus vs Pro deviates; same label does not), never on absent/unknown either side; a real-storage capture retains `accountTier: "Plus"` verbatim and resolves `complete` (no `account_tier_differs`) against a panel that declares no intended tier. Candidate `20260920190000` remains UNAPPLIED; prepared deploy SQL / expected-identity artifacts remain STALE; a real pilot still requires genuine owner approval and a real four weeks; nothing is deployed. Test commands: `npx vitest run src/lib/citation-panel.test.ts src/lib/citation-protocol.test.ts src/lib/citation-protocol.functions.test.ts src/lib/citation-protocol-migration.test.ts`; `npx tsc --noEmit`; and the SCOPED lint on the touched files (the repo-wide lint is separately red and pre-existing).

### Independent verification — 2026-09-21 04:45 UTC

229 focused tests across four files and 6393 full tests across 385 files passed (95.19s full run). TypeScript, scoped ESLint, build and git diff --check passed. Reviewed equivalent-instant intake and bounded actual account tier labels with historical unknown handling. Codex exception: formatter on five changed TS files and this verification record. No production or concurrent-connection proof. Candidate remains unapplied.
