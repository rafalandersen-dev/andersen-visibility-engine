...
beforeEach(async () => {
  await db.exec(
    `RESET ROLE; TRUNCATE backlink_monitoring_requests,backlink_monitoring_limits,ai_expense_requests,ai_expense_budgets,ai_expense_permits; UPDATE auth.users SET deleted_at=NULL,banned_until=NULL; UPDATE workspace_entities SET data='{"websiteUrl":"https://example.com"}';`,
  );
});
...
const reserve = (id = request, who = owner, next = scope) =>
  ...
  }>("SELECT reserve_backlink_monitoring($1,'p',$2,'https://example.com',$3) result", [
    who,
    id,
    next,
  ]);
...
const dispatch = (lease: string) =>
  db.query<{ result: boolean }>(
    "SELECT authorize_backlink_monitoring_dispatch($1,'p',$2,$3) result",
    [owner, request, lease],
  );
const finish = (lease: string, observation: unknown) =>
  db.query("SELECT finish_backlink_monitoring($1,'p',$2,$3,$4)", [
    owner,
    request,
    lease,
    observation,
...
const observation = () => ({
  ...
  complete: false,
  days: [{ date: scope.dateFrom, state: "missing" }],
});
it("binds stable requests and refuses replay changes without exposing leases", async () => {
  ...
  expect(first.record.ceiling_microusd).toBe(24036);
  ...
  await expect(reserve(request, owner, { ...scope, includeSubdomains: true })).rejects.toThrow(
    "backlink_monitoring_replay",
  );
  await expect(reserve(other)).rejects.toThrow("backlink_monitoring_active");
});
...
it("reserves money atomically with a once-only dispatch", async () => {
  ...
  await expect(dispatch(lease)).rejects.toThrow("backlink_monitoring_expense");
  expect(
  ...
  expect(
    ...
    { reserved_microusd: 24036, provider: "dataforseo", operation: "backlink_monitoring" },
  ]);
});
...
it.each([false, true])(
  ...
  async (unknown) => {
    ...
    expect(
      (await db.query("SELECT status,accounting_state FROM backlink_monitoring_requests")).rows,
    ).toEqual([
      {
...
it("preserves valid evidence if expense reconciliation is unavailable", async () => {
  ...
  expect(
    (await db.query("SELECT status,accounting_state,observation FROM backlink_monitoring_requests"))
      .rows,
...
it("holds changed-site or nearly expired requests without money admission", async () => {
  const lease = (await reserve()).rows[0].result.record.lease_token!;
  await fund();
  await db.exec("UPDATE backlink_monitoring_requests SET lease_until=now()+interval '20 seconds'");
  expect((await dispatch(lease)).rows[0].result).toBe(false);
  expect((await db.query("SELECT * FROM ai_expense_requests")).rows).toEqual([]);
});
it("denies direct roles access to request records, limits and functions", async () => {
  for (const role of ["anon", "authenticated", "service_role"]) {
    ...
    await expect(db.query("SELECT * FROM backlink_monitoring_requests")).rejects.toThrow();
...
it.each(["global", "account"])("enforces persistent request quota: %s", async (kind) => {
  await reserve();
  await db.exec("UPDATE backlink_monitoring_requests SET status='held'");
  await db.query(
  ...
  await expect(reserve(other)).rejects.toThrow("backlink_monitoring_quota");
  expect((await db.query("SELECT * FROM backlink_monitoring_requests")).rows).toHaveLength(1);
});
it("retains provider quota after project deletion", async () => {
  ...
  expect((await db.query("SELECT * FROM backlink_monitoring_requests")).rows).toHaveLength(0);
  const bucket = (
...
it("holds a changed website before dispatch and never treats expired dispatched work as retryable", async () => {
  ...
  await db.query("SELECT authorize_backlink_monitoring_dispatch($1,'p',$2,$3)", [
    owner,
    second,
    next,
  ]);
  await db.exec("UPDATE backlink_monitoring_requests SET lease_until=now()-interval '1 second'");
  const history = (
    await db.query<{ result: Array<{ request_id: string; status: string }> }>(
      "SELECT list_backlink_monitoring($1,'p') result",
      [owner],
    )
  ).rows[0].result;
