...
export async function runBacklinkMonitoring(
  userId: string,
  raw: z.infer<typeof backlinkMonitoringRequest>,
  deps: Dependencies = {},
) {
  ...
  if (!["http:", "https:"].includes(website.protocol) || website.username || website.password)
    throw new Error("backlink_monitoring_scope");
  ...
  const reservation = z
    ...
    .parse(
      await call("reserve_backlink_monitoring", {
        ...target,
  ...
  const savedScope = monitoringScope(record.scope, now());
  if (
    record.user_id !== user ||
  ...
  if (
    ...
    throw new Error("backlink_monitoring_unconfirmed");
  ...
  try {
    ...
    const saved = await call("finish_backlink_monitoring", {
      ...attempt,
      p_observation: observation,
    });
    if (saved !== true) throw new Error("backlink_monitoring_unconfirmed");
    ...
    try {
      await call("finish_backlink_monitoring", { ...attempt, p_observation: null });
    } catch {
