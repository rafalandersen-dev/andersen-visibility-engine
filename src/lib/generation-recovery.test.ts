...
describe("recovering one saved generation", () => {
  it("creates an unapproved draft with its stable original identity and preserves other workspace data", () => {
    ...
    const a = (result.data.content as any[])[0];
    ...
    expect((result.data.opportunities as any[])[0]).toMatchObject({
      version: 3,
      status: "drafting",
  ...
  it("opens an existing edited/approved result without overwriting it or creating another copy", () => {
    ...
    Object.assign((data.content as any[])[0], {
  ...
  it("preserves a newer primary draft pointer", () => {
    const data = base();
    (data.content as any[]).push({ id: "newer", projectId: "p" });
    (data.opportunities as any[])[0].currentContentAssetId = "newer";
    const result = recoverGeneratedResultMutation(data, article, now);
    expect((result.data.content as any[]).map((a) => a.id)).toEqual(["newer", "a"]);
    expect((result.data.opportunities as any[])[0].currentContentAssetId).toBe("newer");
  });
  it.each(["missing_project", "missing_opportunity", "archived", "deleted"])(
    ...
    (kind) => {
      ...
      if (kind === "archived") (data.opportunities as any[])[0].archivedAt = now;
      if (kind === "deleted") (data.opportunities as any[])[0].deletedAt = now;
  ...
  it("does not open an unrelated artifact under the same ID", () => {
    const data = withArticle();
    (data.content as any[])[0].projectId = "other";
  ...
  it("attaches a proposed private image once and keeps later owner edits", () => {
    ...
    const a = (first.data.content as any[])[0];
  ...
  it.each([
    ...
    const data = withArticle();
    Object.assign((data.content as any[])[0], patch);
  ...
  it("opening a recovered approved image does not depend on its old private preview", async () => {
    ...
    Object.assign((data.content as any[])[0], { status: "Approved" });
