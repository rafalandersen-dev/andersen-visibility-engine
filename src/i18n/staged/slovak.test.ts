...
import { SK_STAGED_BATCHES, SK_STAGED_CATALOG } from "./sk";
...
it.each(SK_STAGED_BATCHES)("Slovak $name matches reviewed source and parameters", (batch) => {
...
it("keeps staged Slovak outside runtime and assigns each key once", () => {
  expect(isUiLanguage("sk")).toBe(false);
  expect(Object.keys(UI_CATALOGS)).not.toContain("sk");
