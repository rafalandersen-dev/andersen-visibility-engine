...
import { SL_STAGED_BATCHES, SL_STAGED_CATALOG } from "./sl";
...
it.each(SL_STAGED_BATCHES)("Slovenian $name matches reviewed source and parameters", (batch) => {
...
it("keeps staged Slovenian outside runtime and assigns each key once", () => {
  expect(isUiLanguage("sl")).toBe(false);
  expect(Object.keys(UI_CATALOGS)).not.toContain("sl");
...
it("covers the complete current English catalog in staged Slovenian", () => {
  expect(Object.keys(SL_STAGED_CATALOG).sort()).toEqual(Object.keys(UI_CATALOGS.en).sort());
});
