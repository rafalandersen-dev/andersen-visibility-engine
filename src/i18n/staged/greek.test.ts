...
import { EL_STAGED_BATCHES, EL_STAGED_CATALOG } from "./el";
...
it.each(EL_STAGED_BATCHES)("Greek $name matches reviewed source and parameters", (batch) => {
...
it("keeps staged Greek outside runtime and assigns each key once", () => {
  expect(isUiLanguage("el")).toBe(false);
  expect(Object.keys(UI_CATALOGS)).not.toContain("el");
