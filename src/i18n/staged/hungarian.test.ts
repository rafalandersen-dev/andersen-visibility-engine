...
import { HU_STAGED_BATCHES, HU_STAGED_CATALOG } from "./hu";
...
it.each(HU_STAGED_BATCHES)("Hungarian $name matches reviewed source and parameters", (batch) => {
...
it("keeps staged Hungarian outside runtime and assigns each key once", () => {
  expect(isUiLanguage("hu")).toBe(false);
  expect(Object.keys(UI_CATALOGS)).not.toContain("hu");
...
it("covers the complete current English interface key set", () => {
  expect(Object.keys(HU_STAGED_CATALOG).sort()).toEqual(Object.keys(UI_CATALOGS.en).sort());
});
