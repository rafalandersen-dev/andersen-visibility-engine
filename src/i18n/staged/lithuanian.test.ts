...
import { LT_STAGED_BATCHES, LT_STAGED_CATALOG } from "./lt";

const numericTokens = /\b\d+(?:\.\d+)?(?=\b|(?:st|nd|rd|th|d)\b)/g;
// Product names and technical terms that are correctly identical in Lithuanian.
...
it.each(LT_STAGED_BATCHES)("Lithuanian $name matches reviewed source and parameters", (batch) => {
...
it("keeps staged Lithuanian outside runtime and assigns each authored key once", () => {
  expect(isUiLanguage("lt")).toBe(false);
  expect(Object.keys(UI_CATALOGS)).not.toContain("lt");
...
it("covers the complete current English interface key set", () => {
  expect(Object.keys(LT_STAGED_CATALOG).sort()).toEqual(Object.keys(UI_CATALOGS.en).sort());
});
it("does not copy English text into authored Lithuanian values", () => {
  const unchanged = Object.entries(LT_STAGED_CATALOG).filter(
    ([key, value]) => value === UI_CATALOGS.en[key] && !properNames.has(key),
  );
  expect(unchanged.map(([key]) => key)).toEqual([]);
  for (const key of properNames) expect(LT_STAGED_CATALOG[key]).toBe(UI_CATALOGS.en[key]);
});
it("keeps Lithuanian collaborator roles consistent with existing invitation emails", () => {
  for (const role of ["viewer", "editor", "reviewer"] as const)
    expect(LT_STAGED_CATALOG[`collaboration.${role}`]).toBe(euEmailCopy.lt.invitation.roles[role]);
});
