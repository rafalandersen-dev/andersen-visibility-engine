...
import { ET_STAGED_BATCHES, ET_STAGED_CATALOG } from "./et";
...
it.each(ET_STAGED_BATCHES)("Estonian $name matches reviewed source and parameters", (batch) => {
...
it("keeps staged Estonian outside runtime and assigns each key once", () => {
  expect(isUiLanguage("et")).toBe(false);
  expect(Object.keys(UI_CATALOGS)).not.toContain("et");
...
it("covers every current English interface key in staged Estonian", () => {
  expect(Object.keys(ET_STAGED_CATALOG).sort()).toEqual(Object.keys(UI_CATALOGS.en).sort());
});
