...
export function MarketPage({ config }: { config: MarketConfig }) {
  const t = (key: string) => translate(config.lang, key);
  ...
  return (
    <MarketingShell language={config.lang}>
      {/* Hero */}
      <section className="mx-auto max-w-5xl px-6 py-14">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <RegionSelector
            ...
            note={t("publicPricing.regionNote")}
          />
        </div>
        ...
        <div className="mt-7 flex flex-wrap gap-3">
          <Button asChild size="lg" className="gap-2">
            <Link to="/free-ai-visibility-audit">
              {config.ctaAudit} <ArrowRight className="h-4 w-4" />
            </Link>
          </Button>
          <Button asChild size="lg" variant="outline">
            <a href={betaHref}>{config.ctaBeta}</a>
          </Button>
        </div>
      ...
      <section className="mx-auto max-w-5xl px-6 pb-4">
        ...
        <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {config.helps.map((h) => (
            <div
              key={h}
              className="rounded-lg border border-border bg-card p-5 text-sm font-medium"
            >
              {h}
            </div>
          ))}
      ...
      <section className="mx-auto max-w-5xl px-6 pb-12">
          ...
          {PLAN_IDS.map((pid) => (
            <div
              ...
              className={
                ...
                (PLAN_META[pid].recommended ? "border-gold/40 bg-gold/5" : "border-border bg-card")
              }
            >
              ...
              <div className="mt-1 font-display text-xl">
                {formatMoney(planPrice(billingMarket, pid), currency)}
                {pid !== "freePreview" ? (
                  <span className="text-xs text-muted-foreground"> {t("publicPricing.month")}</span>
                ) : null}
              </div>
            </div>
          ))}
        ...
        <div className="mt-3 grid sm:grid-cols-2 gap-3 max-w-2xl text-sm text-muted-foreground">
          <div className="rounded-md border border-border bg-card px-3 py-2">
            {t("publicPricing.setup")}:{" "}
            {formatMoney(addOnPrice(billingMarket, "assistedSetup"), currency)}{" "}
            {t("publicPricing.once")}
          </div>
          <div className="rounded-md border border-border bg-card px-3 py-2">
            {t("publicPricing.care")}:{" "}
            {formatMoney(addOnPrice(billingMarket, "monthlyCare"), currency)}{" "}
            {t("publicPricing.month")}
          </div>
        </div>
        <p className="mt-3 text-sm text-foreground/85">{t("publicPricing.hold")}</p>
        ...
        <ul className="mt-5 grid sm:grid-cols-2 gap-x-8 gap-y-2 max-w-2xl">
          {config.whoFor.map((x) => (
            <li key={x} className="flex gap-2 text-sm text-foreground/85">
              <Check className="h-4 w-4 mt-0.5 text-gold/80 shrink-0" />
              {x}
            </li>
          ))}
        </ul>
      ...
      <section className="mx-auto max-w-5xl px-6 pb-16">
        <div className="rounded-lg border border-gold/40 bg-gold/5 p-8 text-center">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            <Button asChild>
              <a href={betaHref}>
                {config.ctaBeta} <ArrowRight className="h-4 w-4" />
              </a>
            </Button>
            ...
            <Button asChild variant="outline">
              <Link to="/free-ai-visibility-audit">{config.ctaAudit}</Link>
            </Button>
          </div>
        </div>
        ...
        <div className="mt-8">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm">
            <Link to="/beta" className="underline underline-offset-4 hover:text-foreground">
              {t("publicBeta.assistedBeta")}
            </Link>
            <Link to="/case-studies" className="underline underline-offset-4 hover:text-foreground">
              {t("publicBeta.caseStudies")}
            </Link>
            <Link to="/demo-script" className="underline underline-offset-4 hover:text-foreground">
