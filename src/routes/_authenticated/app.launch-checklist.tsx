  ...
  head: () => ({
    meta: [
      { title: "Launch checklist — Milo Growth" },
      {
        name: "description",
        content:
          "Beta launch readiness: setup, content, publishing, measurement, authority and billing status.",
      },
    ],
  }),
...
function StateIcon({ state }: { state: ChecklistState }) {
  if (state === "done")
    return <CheckCircle2 className="h-4 w-4 text-emerald-600 shrink-0" strokeWidth={1.8} />;
  if (state === "optional")
    return <MinusCircle className="h-4 w-4 text-muted-foreground/60 shrink-0" strokeWidth={1.8} />;
...
function OwnerQaPanel({ inputs }: { inputs: LaunchInputs }) {
  ...
  useEffect(() => {
    let cancelled = false;
    getPaddleStatusFn()
      .then((r) => !cancelled && setPaddle(r))
      .catch(() => {});
    getAiRouterStatusFn()
      .then((r) => !cancelled && setAi(r))
      .catch(() => {});
    getGscOAuthStatusFn()
      .then((r) => !cancelled && setGsc(r))
      .catch(() => {});
    return () => {
      cancelled = true;
    };
  ...
  const liveCount = inputs.content.filter(
    (c) => c.livePublishStatus === "published" || c.liveUrl,
  ).length;
  ...
  const rows: { label: string; value: string }[] = [
    ...
    {
      label: t("launch.qa.analyticsEvents"),
      value: inputs.analyticsEventCount === undefined ? "—" : String(inputs.analyticsEventCount),
    },
    { label: t("launch.qa.gscImports"), value: String(gscImports) },
    {
      label: t("launch.qa.gscOAuth"),
      value: gsc ? (gsc.configured ? t("launch.qa.yes") : t("launch.qa.no")) : "…",
    },
    ...
    {
      label: t("launch.qa.gscSyncRows"),
      value: p.gscOAuth?.sync?.lastRowCount != null ? String(p.gscOAuth.sync.lastRowCount) : "—",
    },
    {
      label: t("launch.qa.gscSyncDate"),
      value: p.gscOAuth?.sync?.lastSyncedAt?.slice(0, 10) ?? "—",
    },
    ...
    {
      label: t("launch.qa.aiCandidate"),
      value: ai
        ? ai.candidateConfigured
          ? `${t("launch.qa.yes")} (${ai.candidateModel ?? "?"})`
          : t("launch.qa.no")
        : "…",
    },
    {
      label: t("launch.qa.paddle"),
      value: paddle
        ? paddle.configured
          ? `${t("launch.qa.yes")} (${paddle.environment})`
          : t("launch.qa.no")
        : "…",
    },
  ];
