...
const searchSchema = z.object({
  view: z.enum(["discover", "list", "board", "calendar"]).optional().catch("list"),
  scale: z.enum(["day", "week", "month"]).optional().catch("week"),
  selected: z.string().optional(),
  scheduleAsset: z.string().optional(),
  q: z.string().optional(),
});
...
function PlanPage() {
  ...
  useEffect(() => {
    if (!search.scheduleAsset || !hydrated) return;
    const asset = content.find((item) => item.id === search.scheduleAsset);
    if (asset && pipelineStage({ asset }) === "armed" && asset.scheduledPublishAt) {
      const date = new Date(asset.scheduledPublishAt);
      setDropTime(
        defaultGoLiveLocal(date, new Date(), {
          hours: date.getHours(),
    ...
    navigate({ to: "/app/plan", search: { ...search, scheduleAsset: undefined }, replace: true });
  }, [search.scheduleAsset, hydrated]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      actions={
        <>
          <Button variant="outline" onClick={() => setView("discover")}>
            <MagnifyingGlass size={17} /> {t("today.discover")}
          </Button>
          <Button onClick={openManualForm}>
            <Plus size={17} /> {t("plan.new")}
          </Button>
        </>
      }
      flush
        ...
        <>
          ...
          {showArchived ? (
            ...
            <CalendarView
              ...
              onReschedule={(id) =>
                navigate({ to: "/app/plan", search: { view: "calendar", scheduleAsset: id } })
              }
            />
          ) : (
            <BoardView
        ...
        <section className="rounded-lg border border-[#e2e6eb] bg-[#ffffff]/75 p-5">
          ...
          <label className="mt-3 grid gap-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#5f6872]">
            ...
            <textarea
              ...
              rows={2}
              className="resize-y rounded-md border border-[#e2e6eb] bg-white/70 px-3 py-2 text-xs font-normal normal-case tracking-normal outline-none focus:border-[#076ee5]"
            />
          </label>
...
function SourceCard({ label, status, to }: { label: string; status: string; to: string }) {
  return (
    <Link
      to={to}
      className="grid min-h-[74px] grid-cols-[auto_1fr] gap-2 rounded-md border border-[#e2e6eb] bg-[#fffefa] p-3 transition hover:border-[#91b7e4]"
    >
...
function PlanToolbar({
  ...
  const t = useT();
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#e2e6eb] bg-[#fafbfc] px-5 py-3 md:px-9">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          className={`flex h-10 items-center gap-2 rounded-md border px-3 text-sm ${showArchived ? "border-[#076ee5] bg-[#f4ead4] text-[#765719]" : "border-[#e2e6eb] bg-[#ffffff] text-[#667181]"}`}
        >
          <Archive size={15} /> {showArchived ? t("plan.active") : t("plan.archived")}
        </button>
        <label className="flex h-10 w-[260px] max-w-[55vw] items-center gap-2 rounded-md border border-[#e2e6eb] bg-white px-3 text-[#6f7985]">
...
function ViewTab({
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-pressed={active}
      className={`flex h-10 items-center gap-2 rounded-md border px-3 text-sm transition ${active ? "border-[#e2e6eb] bg-white text-[#202221] shadow-sm" : "border-transparent text-[#667181] hover:bg-white/60"}`}
    >
      <Icon size={15} /> {label}
    </button>
  );
...
function BoardView({
  ...
  return (
    <div
      className={`relative min-h-[calc(100vh-156px)] px-3 py-3 ${selectedId ? "xl:pr-[388px]" : ""}`}
    >
      <div className="overflow-x-auto pb-3">
        <div className="grid w-max grid-flow-col auto-cols-[272px] gap-4">
            ...
            return (
              <section
                ...
                className={`min-h-[calc(100vh-195px)] rounded-lg border bg-[#f8f6f0]/70 px-1.5 pb-3 transition ${
                  dragging
                    ? droppable
                      ? "border-dashed border-[#076ee5] ring-1 ring-[#076ee5]/40"
                      : "border-[#e4ded4] opacity-60"
                    : "border-[#e4ded4]"
                }`}
              >
                <header
                  className="-mx-1.5 mb-2.5 grid grid-cols-[auto_1fr_auto] items-center gap-2 rounded-t-lg border-b border-[#e2e6eb] border-t-[3px] px-2.5 py-2"
...
function OpportunityCard({
  ...
  return (
    <div
      ...
      className={`group relative mb-2 grid w-full cursor-pointer gap-3 rounded-xl border bg-card p-4 text-left shadow-[0_1px_2px_rgba(24,29,31,.03)] transition hover:border-[#c2b7a7] ${selected || checked ? "border-[#076ee5] ring-1 ring-[#076ee5]" : "border-[#ded8ce]"}`}
    >
      <button
        type="button"
        ...
        className={`absolute right-2 top-2 grid h-6 w-6 place-items-center rounded-[3px] border transition ${
          checked
            ? "border-[#a86f09] bg-[#b87f12] text-white opacity-100"
            : "border-[#b3b4b7] bg-white text-transparent opacity-0 group-hover:opacity-100 group-focus-within:opacity-100"
        }`}
      >
        <Check size={11} />
      </button>
      <strong className="pr-5 text-sm leading-[1.45]">
      ...
      <div className="mt-0.5 flex flex-wrap items-center justify-between gap-2">
        ...
        {liveMissing ? (
          <span className="flex items-center gap-1.5">
            ...
            {rewriteEnabled && opportunity.canonicalUrl ? (
              <button
                ...
                className="rounded-[4px] border border-[#e2c9a0] bg-[#f6e9d2] px-1.5 py-1 text-xs font-medium text-[#8a5a12] hover:bg-[#efdcbb]"
              >
          ...
          <button
            ...
            className="rounded-[4px] border border-[#ded8ce] bg-[#f2f5f8] px-1.5 py-1 text-xs font-medium text-[#5c6470] hover:border-[#c2b7a7] hover:bg-[#f1ece1]"
          >
            {t(nextAction(opportunity.pipeline))}
          </button>
        )}
...
function ListView({
  ...
  return (
    <div
      className={`relative min-h-[calc(100vh-220px)] p-5 md:p-8 ${selectedId ? "xl:pr-[388px]" : ""}`}
    >
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        <div
          className="milo-plan-row hidden bg-secondary/40 py-3 text-xs text-muted-foreground md:grid"
          aria-hidden="true"
        >
          <span>{t("plan.work")}</span>
          <span>{t("plan.status")}</span>
          <span>{t("plan.date")}</span>
        </div>
        {opportunities.map((opportunity) => {
          const asset = assets.get(opportunity.id);
          const cover = asset && contentCover(asset);
          const when =
            opportunity.pipeline === "armed"
              ? asset?.scheduledPublishAt
              : opportunity.pipeline === "live"
                ? (asset?.livePublishedAt ?? opportunity.publishedAt)
                : opportunity.dueAt;
          return (
            <button
              key={opportunity.id}
              ...
              className="milo-plan-row"
            >
              <span className="flex min-w-0 items-center gap-4">
                {cover && (
                  <img src={cover} alt="" className="h-14 w-16 shrink-0 rounded-lg object-cover" />
                )}
                <span className="min-w-0">
                  <strong className="block text-sm font-medium leading-6">
              ...
              <span>
                <StageChip stage={opportunity.pipeline} />
              </span>
        ...
        {!opportunities.length && (
          <p className="p-10 text-center text-sm text-muted-foreground">{t("plan.empty")}</p>
        )}
      </div>
    </div>
  );
    ...
    >
      <div className="mx-auto max-w-4xl overflow-hidden rounded-lg border border-[#e2e6eb] bg-[#ffffff]">
        ...
        {opportunities.map((opportunity) => (
          <div
            key={opportunity.id}
            className="flex items-center justify-between gap-4 border-b border-[#e2e6eb] px-5 py-4 last:border-b-0"
          >
            <button
              type="button"
              onClick={() => onSelect(opportunity.id)}
...
function CalendarView({
  ...
  const nextPublication = [...goLives].sort((a, b) =>
    (a.scheduledPublishAt ?? "").localeCompare(b.scheduledPublishAt ?? ""),
  )[0];
  const heading =
    scale === "week"
      ? `${new Intl.DateTimeFormat(locale, { day: "numeric", ...(isSameMonth(weekStart, addDays(weekStart, 6)) ? {} : { month: "short" as const }) }).format(weekStart)}–${new Intl.DateTimeFormat(locale, { day: "numeric", month: "long", year: "numeric" }).format(addDays(weekStart, 6))}`
  ...
  return (
    <div
      className={`relative grid min-h-[calc(100vh-156px)] gap-4 p-5 md:p-8 ${unscheduled.length ? "xl:grid-cols-[minmax(0,1fr)_230px]" : "grid-cols-1"} ${selectedId ? "xl:pr-[388px]" : ""}`}
    >
      <section className="overflow-hidden rounded-lg border border-[#e2e6eb] bg-[#ffffff]">
        <div className="flex min-h-14 flex-wrap items-center justify-between gap-3 border-b border-[#e2e6eb] px-3 py-2.5">
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => move(-1)}
              aria-label={t("plan.previous")}
              className="grid h-9 w-9 place-items-center rounded-md border border-[#e2e6eb]"
            >
              <CaretLeft size={16} />
            </button>
            <button
          ...
          <div className="flex items-center gap-2">
            <div className="flex rounded-md border border-[#e2e6eb] p-0.5">
              {(["day", "week", "month"] as CalendarScale[]).map((item) => (
                <button
                  key={item}
                  type="button"
                  onClick={() => onScale(item)}
                  className={`min-h-9 rounded px-2.5 text-sm capitalize ${scale === item ? "bg-white shadow-sm" : "text-[#64707e]"}`}
                >
                  {t(`plan.${item}`)}
                </button>
              ))}
            ...
            <button
              ...
              className="h-8 rounded-md border border-[#e2e6eb] px-3 text-sm"
            >
              {t("plan.today")}
            </button>
          </div>
        ...
        <div className="overflow-x-auto">
          {scale === "month" ? (
            ...
            <div
              className={`grid ${scale === "day" ? "grid-cols-1" : "min-w-[980px] grid-cols-7"}`}
            >
              {dayDates.map((date) => (
                <CalendarDay
                  key={date.toISOString()}
      ...
      {unscheduled.length > 0 && (
        <aside className="rounded-lg border border-[#e2e6eb] bg-[#ffffff] p-3">
          <div className="flex items-center justify-between">
            <h3 className="font-display text-base">{t("plan.unscheduled")}</h3>
          ...
          <div className="mt-4 grid gap-2">
            {unscheduled.map((opportunity) => (
              <button
                ...
                onClick={() => onSelect(opportunity.id)}
                className="border-t-[3px] border-[#076ee5] bg-[#fafbfc] px-2.5 py-3 text-left"
              >
                <strong className="flex items-center gap-1.5 text-sm leading-4">
          ...
          <div className="mt-5 flex items-center gap-2 border-t border-[#e2e6eb] pt-3 text-sm text-[#697282]">
            <Globe size={14} /> {Intl.DateTimeFormat().resolvedOptions().timeZone}
          </div>
        </aside>
      )}
      {nextPublication && (
        <section className="col-span-full rounded-xl border border-border bg-card p-5">
          <h3 className="mb-4 text-sm font-semibold">{t("today.next")}</h3>
          <div className="flex flex-wrap items-center gap-5">
            {contentCover(nextPublication) && (
              <img
                src={contentCover(nextPublication)}
                alt=""
            ...
            <div className="min-w-0 flex-1">
              ...
              <h4 className="mt-2 text-base font-semibold">{nextPublication.title}</h4>
              <p className="mt-1 text-xs text-muted-foreground">
                {new Intl.DateTimeFormat(locale, {
                  ...
                  hour: "2-digit",
                  minute: "2-digit",
                }).format(new Date(nextPublication.scheduledPublishAt!))}
              </p>
            </div>
            <div className="grid gap-2">
              <Button variant="outline" onClick={() => onOpenAsset(nextPublication.id)}>
                {t("today.review")}
              </Button>
              <Button
                variant="ghost"
                className="text-primary"
                onClick={() => onReschedule(nextPublication.id)}
              >
                {t("today.changeDate")}
              </Button>
            </div>
...
function CalendarDay({
  ...
  return (
    <div
      ...
      className={`${compact ? "min-h-[170px]" : "min-h-[470px]"} border-b border-r border-[#e2e6eb] px-2 pb-3 ${muted ? "bg-[#f2f5f8]/50 text-[#a29d94]" : ""}`}
    >
      <header className="-mx-2 mb-3 grid place-items-center border-b border-[#e2e6eb] px-2 py-2">
        <span className="text-xs uppercase text-[#697282]">
      ...
      {dayGoLives.map((asset) => (
        <button
          ...
          className={`mb-3 grid w-full cursor-grab gap-2 rounded-lg border p-3 text-left transition ${
        ...
        >
          <span className="flex flex-wrap items-center gap-1 text-xs font-medium text-amber-800">
            <Clock size={10} />
            {asset.scheduledPublishAt ? formatTimeLocal(asset.scheduledPublishAt) : ""} ·{" "}
            {t("plan.goesLive")}
          </span>
          <strong className="text-sm leading-[1.4] text-[#3a2f18]">{asset.title}</strong>
          {/* Armed but regressed since arming — the cron WILL refuse this at fire
              time; say so here instead of the morning after. */}
          {riskAssetIds.has(asset.id) ? (
            <span className="flex items-center gap-1 text-xs font-semibold uppercase tracking-[0.08em] text-red-700">
      ...
      {dayGhosts.map((opportunity) => (
        <div
          ...
          className={`mb-2 grid w-full cursor-pointer gap-1 rounded-md border border-dashed bg-[#ffffff]/70 p-2 text-left ${selectedId === opportunity.id ? "border-[#076ee5] ring-1 ring-[#076ee5]" : "border-[#cdc5b7]"}`}
        >
          <span className="text-xs font-medium uppercase tracking-[0.1em] text-[#9a927f]">
            {t("plan.target")}
          </span>
          <strong className="flex items-center gap-1 text-sm leading-[1.4]">
            <span className="min-w-0 [overflow-wrap:anywhere]">{opportunity.title}</span>
            <SampleBadge id={opportunity.id} />
          </strong>
          ...
          {riskOpportunityIds.has(opportunity.id) ? (
            <span className="flex items-center gap-1 text-xs font-semibold uppercase tracking-[0.08em] text-amber-700">
          ...
          {!compact ? (
            <button
              ...
              className="mt-0.5 max-w-full rounded-[4px] border border-[#ded8ce] bg-[#f2f5f8] px-1.5 py-0.5 text-xs font-medium text-[#5c6470] hover:border-[#c2b7a7] hover:bg-[#f1ece1]"
            >
              {t(nextAction(opportunity.pipeline))}
            </button>
          ) : null}
...
function OpportunityDrawer({
  ...
  return (
    <aside
      ...
      className="milo-inspector fixed inset-y-3 right-3 z-30 w-[360px] max-w-[calc(100vw-24px)] overflow-y-auto rounded-lg border border-[#e2e6eb] bg-[#ffffff] p-4 shadow-[-14px_0_30px_rgba(30,34,32,.08)] lg:top-[248px] lg:bottom-3"
    >
      <button
        type="button"
        ref={closeButton}
        onClick={onClose}
        className="absolute right-2 top-2 grid h-10 w-10 place-items-center rounded-lg text-muted-foreground hover:bg-secondary"
        aria-label={t("plan.detail.close")}
      >
        <X size={17} />
      </button>
      <div className="pr-10 text-xs font-medium text-muted-foreground">{t("plan.work")}</div>
      {linkedAsset && contentCover(linkedAsset) && (
        <img
          src={contentCover(linkedAsset)}
          alt=""
      ...
      {linkedAsset?.scheduledPublishAt && stage === "armed" && (
        <p className="mt-4 flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {new Intl.DateTimeFormat(locale, {
            ...
            hour: "2-digit",
            minute: "2-digit",
          }).format(new Date(linkedAsset.scheduledPublishAt))}
        </p>
      )}
      ...
      <div className="mt-5 grid gap-2">
        ...
        {(
          ...
          <Button onClick={() => onOpenEditor(linkedAsset.id)}>
            <FileText size={16} /> {t("today.review")}
          </Button>
        ) : null}
        {stage === "armed" && linkedAsset && (
          <Button
            variant="outline"
            onClick={() =>
              navigate({
                to: "/app/plan",
                search: { view: "calendar", scheduleAsset: linkedAsset.id },
              })
            }
          >
        ...
        {stage !== "parked" ? (
          <Button variant="ghost" className="text-muted-foreground" onClick={archive}>
            <Archive size={15} /> {t("plan.archived")}
          </Button>
        ) : null}
      </div>
      ...
      <details className="mt-5 border-t border-border pt-4">
        ...
        <dl className="my-5 grid gap-2.5">
          ...
          <Detail
            ...
            value={opportunity.ownerName ?? t("plan.detail.unassigned")}
            icon={<UserCircle size={14} />}
          />
          <Detail
            label={t("plan.detail.keyword")}
        ...
        <div className="rounded-md border border-[#e2e6eb] bg-[#f5f7fa] p-3">
          <div className="flex items-center justify-between text-[9px]">
            <span>{t("plan.detail.stage")}</span>
            <strong>
              {inFlow ? `${flowIndex + 1} / ${FLOW_STAGES.length}` : t(`pipeline.stage.${stage}`)}
            </strong>
          </div>
          <div className="my-2 h-1 overflow-hidden rounded-full bg-[#e2e6eb]">
            <span
          ...
          <p className="text-[8px] leading-4 text-[#69727c]">{t("plan.detail.stageBody")}</p>
        </div>

        <div className="mt-2.5 flex items-center gap-3 rounded-md border border-[#e2e6eb] bg-[#f5f7fa] p-3">
        ...
        <p className="mt-1.5 text-[8px] text-[#697282]">{t("plan.detail.scoreBody")}</p>
      </details>
    </aside>
  );
}
